...
class Model(object):
    ...
    # @overload
    # def __init__(self, args: Namespace, device: torch.device = torch.device('cuda'), **kwargs):
    #     self.testset = get_dataset(args.dataset, args.input_size, False)
    #     num_classes = len(self.testset.classes)
    #     sample = self.testset[0][0]
    #     if len(sample.shape) < 3:
    #         channel = 1
    ...
    #         channel = sample.shape[0]
    #
    #     if args.complexity == -1 and channel == 3:
    #         self.model = zoo.get_net(args.model_arch, args.input_size, args.pretrained, num_classes=num_classes)
    #     else:
    #         # net that contains channel/complexity parameters must include arbitrary param discard.
    #         self.model = zoo.get_net(args.model_arch, args.input_size, args.pretrained,
    #                                  num_classes=num_classes, channel=channel, complexity=args.complexity)
    #     self.optimizer = get_optimizer(self.model.parameters(), args.optimizer_choice, args.lr, args.momentum)
    ...
    #     assert osp.exists(args.model_dir)
    #     self.model_dir = args.model_dir
    #     self.batch_size = args.batch_size
    #     self.num_workers = args.num_workers
    #
    #     self.train_criterion = get_criterion(args.train_criterion, args.reduction)
    #     self.test_criterion = get_criterion(args.test_criterion, args.reduction)
